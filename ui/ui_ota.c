...
static pthread_mutex_t ui_ota_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void *ota_check_thread_func(void *arg)
{
    ...
    if (ota_check_update(&info)) {
        /* 有新版本 */
        pthread_mutex_lock(&ui_ota_mutex);
        ...
        pthread_mutex_unlock(&ui_ota_mutex);
...
static void *ota_install_thread_func(void *arg)
{
    ...
    if (ota_check_update(&info)) {
        ...
        /* ota_download_and_apply → ota_apply_app_update → 独立 worker 替换 + 退出进程
         * 正常情况下不会执行到这里 */
    }
...
{
    pthread_mutex_lock(&ui_ota_mutex);
    bool available = last_update_available;
    if (!available) {
        pthread_mutex_unlock(&ui_ota_mutex);
        return false;
    }
    if (version) {
        strncpy(version, last_version, 31);
        version[31] = '\0';
    }
    if (changelog) {
        strncpy(changelog, last_changelog, 511);
        changelog[511] = '\0';
    }
    pthread_mutex_unlock(&ui_ota_mutex);
    return true;
...
void ui_ota_poll(lv_timer_t *timer)
{
    ...
    pthread_mutex_lock(&ui_ota_mutex);
    bool running = ota_running;
    bool update_available = last_update_available;
    char version[sizeof(last_version)];
    strncpy(version, last_version, sizeof(version) - 1);
    version[sizeof(version) - 1] = '\0';
    bool install_mode = ota_install_mode;
    pthread_mutex_unlock(&ui_ota_mutex);
